...
import { ActionSheetController, NavParams, AlertController, App, LoadingController, NavController, Platform, ToastController } from 'ionic-angular';
...
export class PageGmapAutocomplete {
    ...
    error: any;
    switch: string = "map";
    coords = { eventLatitude: '', eventLongitude: '' }
    regionals: any = [];
    currentregional: any;
        ...
        return new Observable((sub: any) => {
            google.maps.event.addListener(autocomplete, 'place_changed', () => {
                ...
                if (!place.geometry) {
                    ...
                    this.coords.eventLatitude = place.geometry.location.lat();
                    this.coords.eventLongitude = place.geometry.location.lng();
                    sub.next(place.geometry.location);
                    sub.complete();
                }
            });
    ...
    SetLocation() {
        ...
        this.callback(this.coords).then(() => { this.nav.pop() });
    }
}
